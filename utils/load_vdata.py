...
class ValData(data.Dataset):
    ...
    def _img_list_retrieve(self):
        ...
        splice_names     = self.img_retrieve('splice_randmask/fake.txt', 'splice_randmask/fake', False)
        ...
        with open(join(self.file_path_fake, 'DDPM_cat.txt')) as f:
            ...
            for content in contents[-int(self.val_num/3):]:
                content = content.strip()
                # img_name = join('/user/guoxia11/cvlshare/DIFFUSION/DDPM/lsun_cat', content)
        ...
        with open(join(self.file_path_fake, 'DDPM_church.txt')) as f:
            ...
            for content in contents[-int(self.val_num/3):]:
                content = content.strip()
                # img_name = join('/user/guoxia11/cvlshare/DIFFUSION/DDPM/lsun_church', content)
        ...
        with open(join(self.file_path_fake, 'DDPM_bedroom.txt')) as f:
            ...
            for content in contents[-int(self.val_num/3):]:
                content = content.strip()
                # img_name = join('/user/guoxia11/cvlshare/DIFFUSION/DDPM/lsun_bedroom', content)
                img_name = join(self.file_path_fake, 'DDPM_bedroom', content)
                DDPM_names.append(img_name)

        # DDPM_names = DDPM_names * 3
        # DDPM_names = DDPM_names[-self.val_num:]
        ...
        with open(join(self.file_path_fake, 'DDIM_bedroom.txt')) as f:
            ...
            for content in contents[-int(self.val_num/2):]:
                content = content.strip()
                # img_name = join('/user/guoxia11/cvlshare/DIFFUSION/DDIM/bedroom/image_samples/images', content)
        ...
        with open(join(self.file_path_fake, 'DDIM_church.txt')) as f:
            ...
            for content in contents[-int(self.val_num/2):]:
                content = content.strip()
                # img_name = join('/user/guoxia11/cvlshare/DIFFUSION/DDIM/church/image_samples/images', content)
                img_name = join(self.file_path_fake, 'DDIM_church', content)
                DDIM_names.append(img_name)

        # guided diffusion
        guided_Diffusion_names = []
        # with open(join(self.file_path_fake, 'D_guided.txt')) as f:
        with open(join(self.file_path_fake, 'GLIDE.txt')) as f:
            contents = f.readlines()
            for content in contents[-self.val_num:]:
                content = content.strip()
                # img_name = join('/user/guoxia11/cvlshare/DIFFUSION/GLIDE', content)
    ...
    def get_mask(self, image_name, cls):
        ...
        ## they are star2, hisd, stylegan2, stylegan3, ddpm, ddim, latent, guided
            ...
            mask = self.load_mask('', real=False, full_syn=True)
            if cls in [10,11]:
