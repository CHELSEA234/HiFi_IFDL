...
def Inference_loc(
    ...
    ## Make a csv file to save the result. 
    args.csv_file_dir = 'lr_' + str(args.learning_rate)+'_loc_csv'
    make_folder(args.csv_file_dir)

    ## Begin the loop.
    for val_tag in [0,1,2,3,4]:

        ## Define the csv file and csv writer. 
        csv_file_name = str(args.initial_epoch) + "_" + str(val_tag) + '.csv'
        csv_file_name = os.path.join(args.csv_file_dir, csv_file_name)
        if os.path.exists(csv_file_name):
            continue
        csv_file = open(csv_file_name, mode='w')
        csv_writer = csv.writer(csv_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        csv_writer.writerow(['image_name', 'AUC', 'F1'])
        ...
        with torch.no_grad():
            ...
            for step, val_data in enumerate(tqdm(val_data_loader)):
                ...
                for img_idx, cur_img_name in enumerate(image_names):
                    ...
                    csv_writer.writerow([image_names[0], AUC_score, F1])
                    csv_file.flush()
